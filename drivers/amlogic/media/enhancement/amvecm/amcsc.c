...
static struct vframe_master_display_colour_s vf_metadata[VD_PATH_MAX];
...
int amvecm_hdr_dbg(u32 sel)
{
	...
	for (k = 0; k < VD_PATH_MAX; k++) {
		pr_err("----HDR VD%d frame info----\n", k + 1);
		pr_err("present_flag:0x%x\n", vf_metadata[k].present_flag);

		if (vf_metadata[k].present_flag == 1) {
			pr_err("----SEI info----\n");
			for (i = 0; i < 3; i++)
				for (j = 0; j < 2; j++)
					pr_err(
						"\tprimaries[%1d][%1d] = %04x\n",
					i, j,
					vf_metadata[k].primaries[i][j]);
			pr_err(
				"\twhite_point = (%04x, %04x)\n",
				vf_metadata[k].white_point[0],
				vf_metadata[k].white_point[1]);
			pr_err(
			"\tmax,min luminance = %08x, %08x\n",
			vf_metadata[k].luminance[0],
			vf_metadata[k].luminance[1]);
			content_light_level =
				&vf_metadata[k].content_light_level;
			pr_err(
