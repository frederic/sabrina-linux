...
#if 1  // LED control for observing video latency
...
#if 1  // LED control for observing video latency
DEFINE_LED_TRIGGER(ledtrig_brightness);
#endif
...
static u32 led_test_enable;
static u32 led_on_period = 10;
static u32 led_off_period = 60;
static u32 led_pattern_enable = 1;
static u32 led_test_cnt;
static u32 led_on_flag;
static u32 revert_clip;
...
SET_FILTER:
	...
	if (led_test_enable) {
		led_test_cnt++;
		if (led_on_flag &&
			(led_test_cnt >= led_on_period)) {
			led_test_cnt = 0;
			led_on_flag = 0;
			/* led off function */
			led_trigger_set_brightness(ledtrig_brightness, 0);
		} else if (!led_on_flag &&
			   (led_test_cnt >= led_off_period)) {
			led_test_cnt = 0;
			led_on_flag = 1;
			/* led on function */
			led_trigger_set_brightness(ledtrig_brightness, 85);
		}
		if (led_pattern_enable) {
			if (led_on_flag) {
				/* White pattern */
				VSYNC_WR_MPEG_REG(
					VPP_VD1_CLIP_MISC0, 0x3ff80200);
				VSYNC_WR_MPEG_REG(
					VPP_VD1_CLIP_MISC1, 0x3ff80200);
			} else {
				/* Black pattern */
				VSYNC_WR_MPEG_REG(
					VPP_VD1_CLIP_MISC0, 0x80200);
				VSYNC_WR_MPEG_REG(
					VPP_VD1_CLIP_MISC1, 0x80200);
			}
			revert_clip = 1;
		}
	} else {
		...
		if (revert_clip) {
			...
			VSYNC_WR_MPEG_REG(VPP_VD1_CLIP_MISC1, 0);
			revert_clip = 0;
		}
	}
...
#if 1  // LED control for observing video latency
	led_trigger_register_simple("led_brightness", &ledtrig_brightness);
#endif
...
MODULE_PARM_DESC(led_test_enable, "\n led_test_enable\n");
module_param(led_test_enable, uint, 0664);

MODULE_PARM_DESC(led_on_period, "\n led_on_period\n");
module_param(led_on_period, uint, 0664);

MODULE_PARM_DESC(led_off_period, "\n led_off_period\n");
module_param(led_off_period, uint, 0664);

MODULE_PARM_DESC(led_pattern_enable, "\n led_pattern_enable\n");
module_param(led_pattern_enable, uint, 0664);

MODULE_PARM_DESC(led_test_cnt, "\n led_test_cnt\n");
module_param(led_test_cnt, uint, 0444);

MODULE_PARM_DESC(led_on_flag, "\n led_on_flag\n");
module_param(led_on_flag, uint, 0444);
