...
static void hdmitx_cedst_stop(struct hdmitx_dev *hdev);
...
static ssize_t show_tmdsrate(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int pos = 0;
	unsigned int idx = 0;
	...
	rate = meson_clk_measure(idx);
	pos += snprintf(buf + pos, PAGE_SIZE, "%d", rate);
	return pos;
}
...
static ssize_t show_sink_feature(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int pos = 0;
	const struct dv_info *dv = &hdmitx_device.rxcap.dv_info;
	...
	if (dv->ieeeoui == DV_IEEE_OUI)
		pos += snprintf(buf + pos, PAGE_SIZE, "DolbyVision/");

	if (prxcap->hdr10plus_info.ieeeoui == HDR10_PLUS_IEEE_OUI)
		pos += snprintf(buf + pos, PAGE_SIZE, "HDR10Plus/");
	if (prxcap->hdr_sup_eotf_smpte_st_2084)
		pos += snprintf(buf + pos, PAGE_SIZE, "HDR10/");
	if (prxcap->hdr_sup_eotf_hlg)
...
static DEVICE_ATTR(sink_feature, 0444, show_sink_feature, NULL);
static DEVICE_ATTR(dc_cap, 0444, show_dc_cap, NULL);
static DEVICE_ATTR(tmdsrate, 0444, show_tmdsrate, NULL);
...
static void hdmitx_cedst_process(struct work_struct *work)
{
	...
	if (ced) {
		hdev->cedst_cnt++;
		hdmitx_set_uevent(HDMITX_CEDST_EVENT, hdev->cedst_cnt);
		pr_info("%s[%d] %d\n", __func__, __LINE__, hdev->cedst_cnt);
	}
...
static int amhdmitx_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(dev, &dev_attr_tmdsrate);
	ret = device_create_file(dev, &dev_attr_dv_cap);
	ret = device_create_file(dev, &dev_attr_sink_feature);
...
static int amhdmitx_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev, &dev_attr_tmdsrate);
	device_remove_file(dev, &dev_attr_dv_cap);
	device_remove_file(dev, &dev_attr_sink_feature);
